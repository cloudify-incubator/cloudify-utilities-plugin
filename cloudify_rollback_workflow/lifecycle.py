...
class UtilitiesLifecycleProcessor(object):
    ...
    #try to use the finisher of uninstall it doesent matter if we perform unlink
    def rollback(self):
        graph = self._process_node_instances(
            workflow_ctx,
...
def rollback_node_instance_subgraph(instance, graph, ignore_failure=True):
    subgraph = graph.subgraph(instance.id)
    sequence = subgraph.sequence()
    ...
    if instance_state not in ['starting']:
        ...
        deletion_validation = []
        stop = [instance.send_event(
            ' Rollback Stop: instance already {0}'.format(instance_state))]
