...
def create_list(ctx, **kwargs):
    ...
    ctx.instance.runtime_properties[FREE_RESOURCES_LIST_PROPERTY] = resource_config
...
def create_list_item(ctx, **kwargs):
    ...
    ctx.instance.runtime_properties[SINGLE_RESERVATION_PROPERTY] = None


@operation(resumable=True)
def reserve_list_item(ctx, **kwargs):
    if not ctx.target.instance.runtime_properties.get(FREE_RESOURCES_LIST_PROPERTY, []):
        raise NonRecoverableError(
            'Reservation has failed, because there are no available resources right now.')

    _refresh_source_and_target_runtime_props(ctx)

    free_resources = ctx.target.instance.runtime_properties.get(FREE_RESOURCES_LIST_PROPERTY)
    ...
    ctx.target.instance.runtime_properties[FREE_RESOURCES_LIST_PROPERTY] = free_resources

    reservations[ctx.source.instance.id] = \
        ctx.source.instance.runtime_properties.get(SINGLE_RESERVATION_PROPERTY)
    ctx.target.instance.runtime_properties[RESERVATIONS_PROPERTY] = reservations
    ...
    ctx.logger.debug('Reservation successful: {0}\
        ...
        ctx.source.instance.runtime_properties.get(SINGLE_RESERVATION_PROPERTY),
        ctx.target.instance.runtime_properties.get(FREE_RESOURCES_LIST_PROPERTY),
        ctx.target.instance.runtime_properties.get(RESERVATIONS_PROPERTY)
    ))
...
def return_list_item(ctx, **kwargs):
    ...
    reservations = ctx.target.instance.runtime_properties.get(RESERVATIONS_PROPERTY, None)
    ...
    ctx.source.instance.runtime_properties[SINGLE_RESERVATION_PROPERTY] = None
